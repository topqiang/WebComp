...
* 
*/
class CoocomController extends Controller{
	...
	public function coocomList(){
		...
		$this->display('coocomList');

	}

	public function coocomAdd(){

		if(empty($_POST)){
			$this->display('coocomAdd');
		}else{
			$upload_res=$this->upload();
			if($upload_res['flag']=='no')$this->error('没有企业图片');
			//存储数据
			$data=array(
				'name'			=>$_POST['name'],
				'href'			=>$_POST['href'],
				'logo'			=>"Uploads/coocom/".$upload_res['result'],
				'status'		=>0,
			);
			$res=$this->nav->add($data);
			if($res){
				$this->success('添加成功',U('Coocom/CoocomList'));
			}else{
				$this->error('添加失败');
			}
		}
	...
	public function coocomEdit(){
		...
		if(empty($_POST)){
			...
			if($upload_res['flag']=='success')$data['logo']="Uploads/coocom/".$upload_res['result'];
			$res=$this->nav->save($data);
			if($res){
				$this->success('修改成功',U('Coocom/coocomList'));
			}else{
				$this->error('修改失败');
			}
		}
	...
	public function coocomDel(){

		if(empty($_GET['id']))$this->error('没有商品id');
		
		$res=$this->nav->save(array('id'=>$_GET['id'],'status'=>"9"));
		if($res){
			$this->success('删除成功',U('Coocom/coocomList'));
		}else{
			$this->error('删除失败');
		}
	...
	public function upload(){
		...
		if($is_upload){
            //load("@.function.php");
			$upload_res=$this->uploadThemeImg('coocom');
